...
def FeasibilityCheckData(infeasibility_count, infeasibility_string):
    ...
    total_supply = 0
    # original i = vertex_list.num_depots + 1 To vertex_list.num_locations
    for i in range(vertex_list.num_depots, vertex_list.num_locations):
    ...
    #i = vertex_list.num_depots + 1 To vertex_list.num_locations
    for i in range(vertex_list.num_depots, vertex_list.num_locations):
        if instance.open_vrp == False:
            ...
            if (vertex_list.vertices[i].mandatory == 1) and (reachable == False):
                ...
                #print(vertex_list.num_customers + 7 + num_stops + infeasibility_count, 1).value = dict("Location ") & 
                print(i - 1)
            ...
            if (vertex_list.vertices(i).mandatory == 1) and (reachable == False):
                infeasibility_count = infeasibility_count + 1
                if infeasibility_count < 5:
                    infeasibility_string = infeasibility_string & dict("Location ") & i - 1 & dict(" cannot be served with the given working time limit.")

                if infeasibility_count == 5:
                    infeasibility_string = infeasibility_string & dict("More can be found under the detected reasons of infeasibility in the solution worksheet.")
    ...
    if instance.backhauls == True:
        ...
        for i in range(vertex_list.num_depots, vertex_list.num_locations):
            if (-1* (vertex_list.vertices[i].pickup_amount) > 0) and (vertex_list.vertices[i].delivery_amount > 0):
                infeasibility_count = infeasibility_count + 1
                if infeasibility_count < 5:
                    infeasibility_string = infeasibility_string + "Location," + "está pedindo coleta e entrega, o que conflita com a opção de backhauls."
                    print(i - 1)
    ...
    if instance.vehicle_location_incompatibility == True:
        ...
        for i in range(vertex_list.num_depots, vertex_list.num_locations):
            ...
            if incompatible_location == True:
                ...
                infeasibility_count = infeasibility_count + 1
                if infeasibility_count < 5:
                    infeasibility_string = infeasibility_string  + "Location " + " não é compatível com nenhum tipo de veículo."
                    print( i - 1)
