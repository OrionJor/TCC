...
def GetArcData():
    ...
    arc_list.distance = [[0]*vertex_list.num_locations for i in range(vertex_list.num_locations)]
    arc_list.duration = [[0]*vertex_list.num_locations for i in range(vertex_list.num_locations)]
    ...
    for i in range(0, vertex_list.num_locations):
        for j in range(0, vertex_list.num_locations):
            ...
            arc_list.duration[i][j] = Time_Converter(dados.iloc[k,3],'mins')
...
def GetSolverOptions():
    ...
    for i in range(Vertex.num_depots ,Vertex.num_locations):
        if Vertex.vertices[i].mandatory == 0:
            fraction_of_optional_customers = fraction_of_optional_customers +1
    
    fraction_of_optional_customers = fraction_of_optional_customers / Vertex.num_customers
    #taxa de remoção
...
def GetVehicleTypeData():
    ...
    for i in range(0, vertex_list.num_depots):
        for j in range(0, base_vehicle_types):
            ...
            work_start_time = Time_Converter(work_s_t, interval = "mins")
            driving_time_limit = Time_Converter(driving_t_l, interval = "mins")
            working_time_limit = Time_Converter(working_t_l, interval = "mins")
    ...
    if instance.vehicle_location_incompatibility == True:
        ...
        vehicle_type_list.compatible = [[0]*vehicle_type_list.num_vehicle_types for i in range(vertex_list.num_locations)]
...
def GetVertexData():
    ...
    for i in range(0, vertex_list.num_locations):
        ...
        time_window_end = Time_Converter(tempo_final, 'mins')
        ...
        service_time = Time_Converter(tempo_servico, 'mins')
